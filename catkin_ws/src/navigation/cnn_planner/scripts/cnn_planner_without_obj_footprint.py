...
class CNNPlanner(object):
	def __init__ (self):
		...
		model_name='resnet18'
		weight_path = os.path.join(dir_pkg,'checkpoints',model_name,model_name+'_last.pt')		
		if weight_path:
	...
	def pose_cb(self,msg):
		...
		for index, (pose, option) in enumerate(zip(global_plan.points, global_plan.road_options)):
			...
			if index==self.next_index_goal_plan:          
				...
				if dist_to_goal<10.:  ## if the ego achieve the goal// the vehicle is in a distance < 10 meter of the point
		...
		if self.stereo_bev_image is None or self.current_pose is None or self.lidar_bev_image is None or self.global_plan is None or self.global_plan.points==[] or self.old_poses_list is None and len(self.raw_path_plan_points)>0:
			# print('return')
			return
		...
		if dist>11.0 or (time_from_last_path > self.time_threshold_to_pub_path \
			...
			test_dataset = CarlaDatset(lidar_bev_image , im_color_lidar , stereo_bev_image , 
				global_plan_points_png , global_plan_line_png , gps_backward_png, root_dir='', train=False)
...
class Resnet18Model(nn.Module):
    ...
    def __init__(self, points_planner,num_in_channels):#cfg: Dict):
        super().__init__()
        
        self.backbone = resnet18(pretrained=False, progress=True)
        num_in_channels = num_in_channels#3 + num_history_channels
...
class CarlaDatset():
    """Face path dataset."""
    def __init__(self, img_lidar , img_lidar_color , img_stereo , img_global_plan_points , 
    	img_global_plan_line , img_gps_backward,root_dir='',  train=True,transform=None):

        self.leaderboard_frame = [img_lidar]
    ...
    def __getitem__(self, idx):
        ...
        mix_inputs = torch.cat([img_lidar_color, img_stereo, img_global_plan_points, img_global_plan_line, img_gps_backward])
