...
# Copyright owned by Universidade de São Paulo
    ...
    xc = np.zeros(number_lines)
    yc = np.zeros(number_lines)
    thetac = np.zeros(number_lines)
    thetac_teste = np.zeros(number_lines)
    kappac = np.zeros(number_lines)
    sigmac = np.zeros(number_lines)
    sc = np.zeros(number_lines)
    ...
    return [xc[0:m+1], yc[0:m+1], thetac[0:m+1], kappac[0:m+1], sigmac[0:m+1], sc[0:m+1]]
