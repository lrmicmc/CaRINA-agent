...
from geometry_msgs.msg import Polygon, PolygonStamped, PointStamped, PoseWithCovarianceStamped, PoseStamped, Point
...
class TrafficLightPosition(object):
	...
	def __init__(self):
		...
		self.lines_to_traffic_signs_pub = rospy.Publisher('/carina/perception/traffic_rules/lines_to_traffic_signs', MarkerArray, queue_size=1)
	...
	def pose_cb(self, msg):
		...
		if self.global_plan==None or len(self.path) <= 0 : 
			return


		pose_gps =  np.array([self.current_pose.pose.pose.position.x, self.current_pose.pose.pose.position.y])

		dist_pose2path = np.sqrt(np.power(self.path[:,0:2]-pose_gps,2).sum(axis=1))
		...
		for index, (pose, option) in enumerate(zip(self.global_plan.points, self.global_plan.road_options)):
			...
			if self.STRAIGHT==option or self.RIGHT==option or self.LEFT==option:
				...
				l_ob = Pointsh(pose.x, pose.y)
				l_ob = l_ob.buffer(1.)###dilated obstacle

				if(l_ob.intersects(self.path_shapely_dilated1)):
					angle = np.arctan2(pose.y-self.current_pose.pose.pose.position.y, pose.x-self.current_pose.pose.pose.position.x) #* 180 / np.pi
	...
	def cnn_path_cb(self, msg):
		if msg.header.stamp > self.path_last_stamp:
			self.path_last_stamp = msg.header.stamp
			arr =  np.array([p.point for p in msg.path])
			self.path = arr[:, 0:2]
...
if __name__ == '__main__':
	print("[Traffic lights poses Node] running...")
	rospy.init_node('traffic_light_poses_node', anonymous=True)
	tfl_p = TrafficLightPosition()
	rospy.spin()